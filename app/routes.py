...
def view_product(id):
    ...
    # Check if the form is validated and the cart exists
    if form.validate_on_submit():
        ...
        # Commit the session changes (if needed)
        session.modified = True
        
        return redirect(url_for('shop'))  # Redirect to the shop page
...
#        'CallBackURL': 'https://sandbox.safaricom.co.ke/mpesa/',
 #       'AccountReference': 'TreasureTwice',
...
@app.route('/dashboard/admin/products', methods=['GET'])
@login_required
def admin_all_products():
    ...
    if search_query:
        ...
        products = ProductsForSale.query.filter(ProductsForSale.name.like(f"%{search_query}%")).all()
    else:
        # If no search query, return all products
        products = ProductsForSale.query.all()

    return render_template('admin_products.html', title="Vendors and Products", products=products, search=search_query)
@app.route('/dashboard/admin/customers', methods=['GET'])
@login_required
def admin_all_customers():
    ...
    if search_query:
        # Search for customers by username or email
        customers = User.query.filter(
            (User.username.like(f"%{search_query}%")) | 
            (User.email.like(f"%{search_query}%"))
        ).filter_by(type='customer').all()
    else:
        # If no search query, show all customers
        customers = User.query.filter_by(type='customer').all()

    return render_template('admin_customers.html', title="All Customers", customers=customers, search_query=search_query)
